...
class Main {
   ...
   private filterEditingTarget: Favorite.Node;
   private useFilter: HTMLInputElement;
   private filterTree: HTMLUListElement;
   ...
   private filterRoot: LayerTree.Filter;
   ...
   private loadAndParse(file_or_url: File | string) {
      ...
      Main.loadAsBlob(p => prog.update(p, 'Receiving file...'), file_or_url)
         .then((o: { buffer: ArrayBuffer|Blob, name: string; }) =>
         this.parse(p => prog.update(p, 'Loading file...'), o))
         .then((obj: any) => { return this.initMain(obj.psd, obj.name); })
         .then(() => {
         prog.close();
   ...
   private initMain(psd: psd.Root, name: string) {
      let deferred = m.deferred();
      setTimeout(() => {
         try {
            this.renderer = new Renderer.Renderer(psd);
            let layerTree = <HTMLUListElement>document.getElementById('layer-tree');
            this.layerRoot = new LayerTree.LayerTree(this.optionSafeMode.checked, layerTree, psd);
            this.filterRoot = new LayerTree.Filter(this.filterTree, psd);
            for (let key in this.renderer.nodes) {
               if (!this.renderer.nodes.hasOwnProperty(key)) {
                  continue;
               }
               ((r: Renderer.Node, l: LayerTree.Node) => {
                  r.getVisibleState = () => l.checked;
               })(this.renderer.nodes[key], this.layerRoot.nodes[key]);
            }
            layerTree.addEventListener('click', e => {
               let target = e.target;
               if (target instanceof HTMLInputElement && target.classList.contains('psdtool-layer-visible')) {
                  let n: LayerTree.Node = this.layerRoot.nodes[parseInt(target.getAttribute('data-seq'), 10)];
   ...
   private initFavoriteUI(): void {
      ...
      let updateFilter = (): void => {
         let node: Favorite.Node = this.filterEditingTarget;
         if (!this.useFilter.checked) {
            this.favorite.update({ id: node.id, type: 'folder' });
            this.favorite.updateLocalStorage();
            return;
         }
         let s = this.filterRoot.serialize();
         if (!s) {
            this.favorite.update({ id: node.id, type: 'folder' });
            this.favorite.updateLocalStorage();
            return;
         }
         this.favorite.update({ id: node.id, type: 'filter', data: { value: s } });
         this.favorite.updateLocalStorage();
      };
      this.useFilter = Main.getInputElement('#use-filter');
      this.useFilter.addEventListener('click', e => {
         let inp = e.target;
         if (inp instanceof HTMLInputElement) {
            if (inp.checked) {
               this.filterTree.classList.remove('disabled');
            } else {
               this.filterTree.classList.add('disabled');
            }
            updateFilter();
         }
      }, false);
      ...
      if (elem instanceof HTMLUListElement) {
         this.filterTree = elem;
      } else {
         throw new Error('element not found #filter-tree');
      }
      this.filterTree.addEventListener('click', e => {
         let inp = e.target;
         if (inp instanceof HTMLInputElement) {
      ...
      jQuery('#filter-dialog').on('shown.bs.modal', e => {
         let parents: string[] = [];
         for (let p of this.favorite.getParents(this.filterEditingTarget)) {
            if (p.type === 'filter') {
               parents.push(p.data.value);
            }
         }
         if (this.filterEditingTarget.type === 'filter') {
            this.useFilter.checked = true;
            this.filterTree.classList.remove('disabled');
            this.filterRoot.deserialize(this.filterEditingTarget.data.value, parents);
         } else {
            this.useFilter.checked = false;
            this.filterTree.classList.add('disabled');
            this.filterRoot.deserialize('', parents);
         }
         let inputs = <NodeListOf<HTMLInputElement>>this.filterTree.querySelectorAll('input');
   ...
   private initUI() {
      ...
      this.save = filename => saveAs(new Blob([
   ...
   private static removeAllChild(elem: HTMLElement): void {
